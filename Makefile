...
hostname: ## shows local machine ip
	...
	cd infrastructure/nginx-php && $(MAKE) container-ip
...
host-check: ## shows this project ports availability on local machine
	cd infrastructure/mysql && $(MAKE) port-check
...
project-destroy: ## removes the project from Docker network destroying its data and Docker image
	cd infrastructure/mysql && $(MAKE) clear destroy
...
database-ssh: ## enters the backend container shell
	cd infrastructure/mysql && $(MAKE) ssh
